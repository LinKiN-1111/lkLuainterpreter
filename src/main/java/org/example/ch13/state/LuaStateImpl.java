package org.example.ch13.state;

import org.example.ch13.Chunk.BinaryChunk;
import org.example.ch13.Chunk.Prototype;
import org.example.ch13.Chunk.Upvalue;
import org.example.ch13.api.*;
import org.example.ch13.vm.Instruction;
import org.example.ch13.vm.OpCode;
...
import static org.example.ch13.api.ThreadStatus.LUA_ERRRUN;
import static org.example.ch13.api.ThreadStatus.LUA_OK;
import static org.example.ch13.state.LuaValue.converToFloat;
...
public class LuaStateImpl implements LuaState, LuaVM {
    ...
    public void arith(ArithOp op) {
        ...
        Object result = Arithmetic.arith(x, y, op,this);
        if (result != null) {
            stack.push(result);
        } else {
    ...
    public void concat(int n) {
        if (n == 0) {
            ...
            for (int i = 1; i < n; i++) {
                ...
                Object x = stack.pop();
                Object y = stack.pop();
                Object mm = getMetamethod(x, y, "__concat");
                if (mm != null) {
                    stack.push(callMetamethod(x, y, mm));
                    continue;
                }
    ...
    //从表里取值的方法做一层抽象,以便复用
    private LuaType getTable(Object table, Object index, boolean raw) {
        if(table instanceof LuaTable){
            LuaTable luaTable = (LuaTable) table;
            Object v = luaTable.get(index);
        ...
        if(!raw){
            Object metafield = this.getMetafield(table, "__index");
            if(metafield != null) {
                if(metafield instanceof LuaTable) {
                    return this.getTable((LuaTable) metafield, index,false);
                }else if(metafield instanceof Closure) {
                    Object v = callMetamethod(table, index, metafield );
                    stack.push(v);
                    return LuaValue.typeOf(v);
                }
            }
    ...
    public void setTable(int idx) {
        ...
        setTable(t,key,value,false);
    }

    private void setTable(Object table, Object key, Object value,boolean raw) {
        if(table instanceof LuaTable){
            LuaTable luaTable = (LuaTable) table;
            Object v = luaTable.get(key);
            if(raw || v != null || !luaTable.hasMetafield("__newindex")){
        ...
        if(!raw){
            Object metafield = this.getMetafield(table, "__newindex");
            if(metafield != null) {
                if(metafield instanceof LuaTable) {
                    this.setTable((LuaTable) metafield,key,value,false);
                }
                if(metafield instanceof Closure) {
                    Object closure = callMetamethod(table, key, metafield);
    ...
    public void call(int nArgs, int nResults) {
        if(stack.isValid(-(nArgs+1))){
            ...
            Object f = o instanceof Closure ? o : null;

            if(f == null){
                Object metafield = this.getMetafield(o, "__call");
                if(metafield != null && metafield instanceof Closure) {
                    stack.push(f);
                    insert(-(nArgs+2));
                    nArgs += 1;
                    f = metafield;
            ...
            if(f!=null){
                ...
                if(((Closure) closure).getProto()!=null){
    ...
    private void callJavaClosure(int nArgs, int nResults, Closure closure) {
        ...
        r = closure.getJavaproto().invoke(this);
...
//            printStack();
    ...
    private LuaTable getMetatable(Object val) {
        if (val instanceof LuaTable) {
            return ((LuaTable) val).getMetaTable();
        }
        String key = "_MT" + LuaValue.typeOf(val);
        if((LuaTable) registry.get(key) != null){
            return ((LuaTable) registry.get(key)).getMetaTable();
        }
        return null;
    }
    ...
    public boolean getMetatable(int idx) {
        Object o = this.stack.get(idx);
        LuaTable metatable = getMetatable(o);
        if(metatable != null){
            this.stack.push(metatable);
            return true;
        }else{
            return false;
        }
    ...
    public void setMetatable(int idx) {
        Object mt = this.stack.pop();
        Object o = this.stack.get(idx);
        if(mt == null){
            this.setMetatable(o,null);
        }else if(mt instanceof LuaTable){
            this.setMetatable(o,(LuaTable)mt);
        }else{
            throw new RuntimeException("Invalid metatable");
        }
    }
    ...
    public boolean next(int idx) {
        Object table = stack.get(idx);
        if (table instanceof LuaTable) {
            Object key = this.stack.pop();
            LuaTable table1 = (LuaTable) table;
            Object nextKey = table1.nextKey(key);
            //结束条件
            if (nextKey != null) {
                this.stack.push(nextKey);
                this.stack.push(table1.get(nextKey));
                return true;
            }
            return false;
        }
    ...
    public ThreadStatus pCall(int nArgs, int nResults, int msgh) {
        ...
        try {
            ...
            stack.push(e.getMessage());  //压入相应的错误
            return LUA_ERRRUN;
        }
